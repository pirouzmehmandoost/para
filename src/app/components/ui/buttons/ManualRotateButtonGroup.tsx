...
const ManualRotateButtonGroup = memo(({ callback, active, rotation, clickedButtonID}: ManualRotateButtonGroupProps) => {
  ...
  const rotateActionButtonValues = useMemo(() => {
    return {
      TOP: {
        text: 'TOP',
        callback: () => callback({ x: rotation.x + Math.PI / 2, y: rotation.y, z: rotation.z }, 'rotation-button-top'),
        injectStyle: `translate-x-0 -translate-y-3.25 rotate-x-60 rotate-y-0 rotate-z-45 contrast-200`,
        id: 'rotation-button-top'
      },
      FRONT: {
        text: 'FRONT',
        callback: () => callback({ x: rotation.x, y: rotation.y + Math.PI / 2, z: rotation.z }, 'rotation-button-front'),
