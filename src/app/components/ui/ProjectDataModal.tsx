...
import { memo, useCallback, useLayoutEffect, useState, useMemo } from 'react'
...
interface ManualRotationButtonProps extends BaseButtonProps {
  callback: () => void
  defaultRotationAnimationActive: boolean
}

const ManualRotationButton = memo(({
  callback,
  defaultRotationAnimationActive,
  buttonStyle,
  text
}: ManualRotationButtonProps) => {
  return (
    <button
      className={`absolute appearance-none w-10 h-10 backdrop-blur-md cursor-pointer text-center text-xs sm:text-xs md:text-sm ${buttonStyle} transition-all duration-500 ease-in-out ${!defaultRotationAnimationActive ? 'bg-neutral-300/60 hover:bg-neutral-400/60' : 'bg-neutral-500/50 hover:bg-neutral-400/50'}`}
      onClick={callback}
      type='button'
    >
      {text}
...
const BackButton = memo(({ callback }: BackButtonProps) => {
  return (
...
const DetailsPanel = memo(({ expanded, care = '', description = '', dimensions = '', materialSpecs = '', weight = '' }: DetailsPanelProps) => {
  ...
  return (
    <div className={`flex flex-col w-fit h-full space-y-4 items-center-safe select-none transition-all transition-discrete duration-200 ease-in-out ${expanded ? 'opacity-100' : 'opacity-0'}`}>
      <div className='max-w-5/6 h-full text-center'>
        {description}
      </div>
      <div className='max-w-fit h-full text-neutral-900 text-sm text-left font-medium whitespace-nowrap'>
    ...
    <div className='flex flex-col w-full h-fit p-6 justify-center-safe items-center-safe rounded-4xl backdrop-blur-md backdrop-brightness-200'>
      <div className='flex flex-row w-full h-fit space-x-4 justify-center place-items-center-safe'>
        ...
        <div className='flex flex-col w-full items-center place-content-center justify-center'>
          ...
          <div className={`transition-all duration-500 ease-in-out text-neutral-900 ${autoRotateActive ? 'animate-pulse invert-75' : 'invert-0'}`}>
            Auto
          </div>
        </div>
          ...
          <div className='flex flex-row w-full h-full lg:place-self-start xl:place-self-start 2xl:place-self-start space-x-4 items-end-safe justify-between lg:justify-start xl:justify-start 2xl:justify-start'>
            <div className='flex flex-col w-fit h-full space-y-2'>
              ...
              <RotationControlsPanel
                rotation={rotation}
                handleAutoRotate={toggleAutoRotation}
