    ...
    <div className='flex flex-col grow place-items-start max-w-fit max-h-fit p-6 text-link select-none'>
      <div className='fixed z-30'>
        <ActionButton
          ...
          callback={(e) => handleClick(e, !visible)}
        >
          {visible === false ? <MenuIcon fontSize='medium' /> : <CloseIcon fontSize='medium' />}
        </ActionButton>
      </div>
      <div className={`flex grow flex-col max-w-fit max-h-fit z-20 p-4 space-y-6 rounded-3xl bg-container/20 origin-top backdrop-blur-md justify-center-safe items-center-safe transition-all transition-discrete duration-500 ease-in-out ${visible ? 'visible opacity-100 pointer-events-auto' : 'collapse opacity-0 pointer-events-none'}`}>
