...
const AnimatedLight = (props) => {
  ...
  useFrame(({ clock, camera }, delta) => {
    ...
    const pointLightMovement = 25 * (Math.sin(elapsedTime) % 1);
    ...
    switch (type) {
      case 'pointLight':
        yOffset = 0;
        zOffset = 60;
        x += pointLightMovement;
        y = (camera.position.y + yOffset) - 0.25 * (sinMovement * (0.25 * pointLightMovement));
        z = (camera.position.z - zOffset - 10);
        break;
