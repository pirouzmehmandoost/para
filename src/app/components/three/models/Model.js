...
const { OFFSET_CAMERA_POSITION: [, , cameraOffsetDistance] } = carouselConfigs
...
const Model = (props) => {
  ...
  function easeMaterialProperties(materialToUpdate, delta) {
    ...
    if (Math.abs(animateMaterialRef.current.ior - materialToUpdate.ior) > EPSILON_10e4) easing.damp(animateMaterialRef.current, "ior", materialToUpdate.ior, 0.3, delta)
  ...
  function updateDeterministicMaterialProperties(materialToUpdate) {
    ...
    // NOTE: For now these 3 texture map slots on any material in materialStore are never non-null. Set needsUpdate = true anyway.
  ...
  useFrame(({ camera: cam }, delta) => {
    ...
    const rotationMode = (
      !selectedAndFocused || !animateRotation ?
        RotationAnimationModes.MODE_IDLE :
        selection.defaultRotationAnimationActive ?
          RotationAnimationModes.MODE_TURNTABLE : RotationAnimationModes.MODE_MANUAL
    )
    ...
    if (!materialReadyRef.current) {
      // One-shot to seed animateMaterialRef from the store's default material
